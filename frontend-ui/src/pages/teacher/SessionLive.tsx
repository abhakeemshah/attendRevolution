...
// Session data structure passed from CreateSession - matches backend fields
...
export default function SessionLive() {
  ...
  // Timer state (optional, derived from start/end times if provided)
  ...
  useEffect(() => {
    if (sessionData) {
      // If start/end available, compute total seconds as best-effort
      if (sessionData.startTime && sessionData.endTime) {
        try {
          const [sh, sm] = sessionData.startTime.split(":").map(Number);
          const [eh, em] = sessionData.endTime.split(":").map(Number);
          const start = sh * 60 + sm;
          const end = eh * 60 + em;
          const secs = Math.max(0, (end - start) * 60);
      ...
      // Generate QR placeholder string.
      // TODO: Request QR from backend: POST /api/v1/sessions/:id/qrcode
      // Example: const res = await fetch(`/api/v1/sessions/${sessionData.id}/qrcode`, { method: 'POST' })
      setQrValue(`QRPLACEHOLDER:${sessionData.id}`);
      // Attempt to fetch QR payload from backend via centralized helper (feature-flagged in helper)
      (async () => {
        try {
          const api = (await import("@/lib/api")).default;
          const res = await api.fetchQr(sessionData.id);
          if (!res.ok) {
            setQrError(res.message ?? "Failed to get QR token");
            return;
          }
          setQrError(null);
          if (res.data) {
            // helper returns either string token or { qrToken }
            if (typeof res.data === "string") setQrValue(res.data);
            else if (res.data.qrToken) setQrValue(res.data.qrToken);
          }
        } catch (e: unknown) {
          console.warn("QR fetch failed", e);
          setQrError("Failed to fetch QR token");
        }
      })();
    }
  }, [sessionData]);
  ...
  }, [isActive, timeRemaining, endSession]);

  // Real-time attendance marking will be handled by backend websocket or polling.
  // Currently no automatic simulation here; UI shows placeholders.
  ...
  const endSession = useCallback(() => {
    ...
    // Placeholder: POST attendance to backend
    // TODO: POST /api/v1/attendance with sessionId and attendance records
    console.log("[TODO] POST /api/v1/attendance ->", {
  ...
  const downloadReport = () => {
    // Placeholder: In production request report from backend
    // TODO: GET /api/v1/sessions/:id/report?format=csv or ?format=pdf
    // Example: GET /api/v1/sessions/:id/report?format=csv
    (async () => {
      try {
        setSessionEnded(true); // keep UI consistent while downloading
        const api = (await import("@/lib/api")).default;
        const res = await api.fetchReport(sessionData!.id, "csv");
  ...
  // If an expectedBatch exists, use it to render boxes; otherwise show placeholder
  const rollBoxes = Array.from({ length: sessionData.expectedBatch ?? 30 }, (_, i) => i + 1);
        ...
        <div className="container mx-auto">
          ...
          <div className="mb-4 text-center">
            ...
            <p className="text-muted-foreground text-sm">
              {sessionData.class} {sessionData.group && `(${sessionData.group})`} • {sessionData.sessionType} • {sessionData.sessionDate}
            </p>
          </div>
            ...
            <div className="space-y-4 order-1 lg:order-2">
              ...
              <Card className="bg-card border-border">
                ...
                <CardContent className="flex flex-col items-center pb-6">
                  ...
                  {/* QR error display */}
                  {qrError && (
                    <div className="absolute bottom-2 left-2 right-2 p-2 bg-secondary/80 rounded text-sm text-destructive flex items-center gap-2">
